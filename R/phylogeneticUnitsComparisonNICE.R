...
## RACs with lines 
RAC_plots_nice <- nice_ulrb_rarefied %>% 
...
## Silhouette scores with lines plot
# silhouette scores
sil_plots_nice <- nice_ulrb_rarefied %>% 
  mutate(Type = factor(Type, levels = c("ASV", "OTU", "mOTU"))) %>% 
  mutate(Group = paste(Sample, Classification, sep = "_")) %>% 
  group_by(Type, Group) %>% 
...
# Combine RACs with Silhouette scores
gridExtra::grid.arrange(RAC_plots_nice, sil_plots_nice)
